...
class DataEnrichmentError(TarzanError):
...
class InsufficientDataError(TarzanError):
...
class MetricCalculationError(TarzanError):
...
class ClassificationError(TarzanError):
...
class ConfigurationError(TarzanError):
