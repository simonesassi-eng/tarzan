...
def test_resolve_inputs_local(monkeypatch, tmp_path):
    ...
    # Present → resolved, so the rebalancer gets its per-holding targets.
    tph = tmp_path / "targets_per_holding.csv"
    tph.write_text("name,isin,ticker,target_portfolio\n", encoding="utf-8")
    monkeypatch.setenv("TARGETS_PER_HOLDING_PATH", str(tph))
    assert delivery.resolve_inputs()["targets_per_holding"] == str(tph)
