...
def _system_prompt(language: str, today_str: str) -> str:
    return (
        ...
        "- Start EVERY line with a short time tag in square brackets: an "
        "exact time (e.g. '[14:30]') ONLY if the source clearly states it, "
        "otherwise the coarser '[Today]' or '[Yesterday]'"
        f" (translate the tag word into {language})"
        ". Never guess a precise time you don't actually know — the coarse "
        "tag is always safer than a fabricated one.\n"
        "- After the tag, one self-contained, headline-style sentence, "
        "roughly 12-22 words. No headings, no horizon labels beyond the "
        "time tag, no numbering, no markdown, no bullet characters.\n"
