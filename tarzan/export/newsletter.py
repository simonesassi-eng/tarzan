...
def _eur_smart(amount: Optional[float], signed: bool = False) -> str:
    """Compact EUR formatter: shows ``€9.6k`` / ``<amount>`` / ``€1.2M``
    instead of the long-form ``<amount>``. Useful in dense rows where
...
def _build_hero(ctx: _NewsletterContext) -> dict:
    ...
    return {
        # Hero big number keeps the full amount (<amount>): the
        # entire visual hierarchy of the Status section depends on it.
        "total_value": _eur(m.total_value),
        # Everything else uses the compact form (€9.6k / <amount> / €1.2M)
