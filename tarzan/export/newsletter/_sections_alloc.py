...
def _build_hero(ctx: _NewsletterContext) -> dict:
    ...
    # different spans, which is how a -0.18% session came to print -<amount>.)
    #
    ...
    def _pnl_tile(label, eur, pct, denominator):
        ...
        the book rather than the one that says how the book is doing: +<amount>
    ...
    return {
        # Hero big number, rounded to whole euros (<amount>): decimals add
...
def _div_table(rows: list[dict], tol: float, base: Optional[float] = None,
    ...
    amount inline (e.g. "26.5% · <amount>") — same row height, no extra
...
def _build_diversification(ctx: _NewsletterContext) -> dict:
    ...
    # Emerging Markets read <amount> while its sole holding, XMME, was worth
    # <amount> on its own.
