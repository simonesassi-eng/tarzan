...
class ValuationCompletenessEvaluator:
    ...
    def _kind(holding: object) -> str:
        ...
        resolution = TypeEvidenceGateway().resolve(
            ...
            getattr(holding, "asset_type", None),
        )
    ...
    def evaluate(self, holdings: list[object], ledger) -> ValuationCompletenessAssessment:
        ...
        for holding in holdings:
            ...
            basis = abs(anchor_value) if anchor_value not in (None, 0.0) else (
                abs(cost_basis) if cost_basis is not None else None
            )
            ...
            has_primary_price = self._finite(getattr(holding, "current_price", None)) is not None
        ...
        missing_basis = sum(
            float(row["evidence"].materiality_basis_eur or 0.0)
            for row in missing_rows
        )
        missing_pct = (
            missing_basis / total_basis * 100.0
            if total_basis > 0
            else None
        )
        material_rows = []
        for row in missing_rows:
            ...
            policy = row["policy"]
            item_pct = (
                float(evidence.materiality_basis_eur or 0.0) / total_basis * 100.0
                if total_basis > 0
                else None
            )
            if item_pct is None or item_pct > policy.valuation_materiality_pct:
        ...
        if material_rows:
            critical_ref = ledger.open_failure(
                ...
                context={
                    ...
                    "applied_policy_ids": [row["policy"].policy_id for row in material_rows],
                },
            )

        all_evidence = tuple(row["evidence"] for row in rows)
        failure_refs = tuple(degraded_failure_refs + ([critical_ref] if critical_ref else []))
        if critical_ref:
