...
from portfolio_analyzer.models.holding import AssetClass, Holding
...
class MetricsEngine:
    ...
    def _rebalancing(self, ctx: dict) -> None:
        if self.config is None:
            ...
            return
        from portfolio_analyzer.engine.rebalancer import compute_unified_rebalancing
    ...
    def _geo_benchmark(self, ctx: dict) -> None:
        from portfolio_analyzer.data.geo_resolver import lookup_geo_by_index_name
...
def _fetch_benchmark_history(ticker: str) -> pd.Series:
    from portfolio_analyzer.data.enricher import _fetch_ticker_data, convert_to_eur
